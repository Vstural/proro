...
import (
	...
	"proro/pkg/rtmp/av"
	"proro/pkg/rtmp/core"
	"proro/pkg/rtmp/utils/uid"
	"reflect"
	"strings"
	"time"

	"proro/pkg/rtmp/container/flv"
)
...
func (s *Server) handleConn(conn *core.Conn) error {
	...
	appname, publishinfo, tcurl_name := connServer.GetInfo()
	fmt.Println("Got appname: ", appname)
	fmt.Println("Got publishinfo: ", publishinfo)
	fmt.Println("Got appname: ", tcurl_name)
...
func (s *Server) GetStream() *RtmpStream {
	return s.handler
}

// func (s *Server) ResolveHandshake(conn *core.Conn) {
// 	return
// }

// func (s *Server) ServeConn(conn *core.Conn) {
